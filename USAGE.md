...
The `zails` binary is a standalone executable with no dependencies. Choose one of:
...
# Download the tarball for your platform, extract, and add to PATH.
```

### Build Requirements

To build projects generated by `zails init`, you need:
...
If you are contributing to the Zails framework itself:
...
This builds both the framework and the CLI tool at `./zig-out/bin/zails`.
...
│   ├── echo_handler.zig
│   └── ping_handler.zig
├── server/             # Framework internals (embedded in the CLI)
│   ├── config.zig
...
Two example handlers are included: `echo_handler` (MESSAGE_TYPE=1) and `ping_handler` (MESSAGE_TYPE=2).
...
```bash
zails create handler my_handler
zails build
```

This creates `handlers/my_handler.zig` with a unique MESSAGE_TYPE and the required exports. Running `zails build` regenerates `handlers/mod.zig` and compiles the project.

### By Hand

1. Create `handlers/my_handler.zig`:
...
2. Rebuild (auto-regenerates `handlers/mod.zig`):

```bash
zails build
```

Your handler is now automatically:
...
| Command | Description |
|---------|-------------|
| `zails init <name>` | Create a new project with server framework, example handlers, and build config |
| `zails build` | Regenerate `handlers/mod.zig` from handler files and run `zig build` |
| `zails create handler <name>` | Generate a handler with unique MESSAGE_TYPE |
| `zails create model <name> [--table=...]` | Generate an ORM model for ClickHouse |
| `zails create service <name>` | Generate a gRPC service handler |
| `zails create migration <name>` | Generate a timestamped database migration |
| `zails create config` | Generate `config/` directory with YAML configuration files |
| `zails scaffold <name> [--fields=...]` | Generate model + migration + service in one command |
